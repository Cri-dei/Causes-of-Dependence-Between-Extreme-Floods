...
setwd(paste0("C:/Users/user/Desktop/REGIONAL-QQ/Lagtime_",Lag_time,"/Plot"))
#setwd(paste0("C:/Users/39349/Documents/Regional/Lag time/Lagtime_",Lag_time,"/Plot"))

pdf(file=paste0("50ALTIT_MIN_hh3",pvalue,"_Lagtime_",Lag_time,".pdf"),width=9, height=9)  
...
#for(hh in 2:11){
hh<-3
  k<-combinations(6, hh, v=v1, set=TRUE, repeats.allowed=FALSE)
    ff<-1
  for(ff in 1:nrow(k)){
 
    Density_perc$CODE[xxx]<-paste(hh,ff)
    Density_perc$Selected[xxx]<-paste(colnames(M_ALL_0.01_POS_perc)[c(k[ff,])], collapse = ' , ')
    colnames(M_ALL_0.01_POS_perc)[c(k[ff,])]
    
    M1_DEP_0[,2]<- apply(M_ALL_0.01_POS_perc[,c(55,57,40,43,k[ff,])],1,min)   
    M1_IND_0[,2]<- apply(M_ALL_IND_perc0.01[,c(55,57,40,43,k[ff,])],1,min)   
    ...
    title=print(paste0("Hydrological Indexes selected: ",colnames(M_ALL_0.01_POS_perc[,c(56,57,40,43,k[ff,])])))
    
    Final_predictors<-colnames(M_ALL_0.01_perc[,c(55,57,40,43,k[ff,])])
    nam0<-paste(colnames(M_ALL_IND_perc)[c(55,57,40,43,k[ff,])], collapse = ' , ')
    ...
    boxplot(M1_DEP$Distance[which(M1_DEP$Class=="Depdep")],M1_DEP$Distance[which(M1_DEP$Class=="Depind")],M1_DEP$Distance[which(M1_DEP$Class=="High Syn")],
            names=c("DepInd","Depdep","High Syn"), main="Distance Groups")
