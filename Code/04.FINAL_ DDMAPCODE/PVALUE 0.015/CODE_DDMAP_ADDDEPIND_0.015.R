...
#The final matrix are:
#M_ALL_IND_perc: for indipendent couples
#M_ALL_0.015_POS_perc : for dependent couples

##################################
############################# CHANGING THRESHOLD PVALUE #################################
#Selection as dependent couples with pvalue:0.015

for( i in 1:nrow(M_ALL_0.015_POS_perc))
{
  if(M_ALL_0.015_POS_perc$KendalT.p.value[i]<=0.015){M_ALL_0.015_POS_perc$X[i]<-2}
  else{M_ALL_0.015_POS_perc$X[i]<-1}
}
...
# for( i in 1:nrow(M_ALL_0.015_POS_perc))
# {
#   if(M_ALL_0.015_POS_perc$Num_Asyncr_occ[i]<20){M_ALL_0.015_POS_perc$X.1[i]<-NA}
#   else{ if(M_ALL_0.015_POS_perc$KT_pvalue_Asy[i]<=0.015){M_ALL_0.015_POS_perc$X.1[i]<-2} else{M_ALL_0.015_POS_perc$X.1[i]<-1}
#   }
# }
# 
...
M_ALL_0.015_POS_perc$Class[M_ALL_0.015_POS_perc$N.SY.N.ALL>=0.60]<-"High Syn"
M_ALL_0.015_POS_perc$X.1[M_ALL_0.015_POS_perc$N.SY.N.ALL>=0.60]<-c(3)
M_ALL_0.015_POS_perc$Class[M_ALL_0.015_POS_perc$X.1==1]<-"Depind"
M_ALL_0.015_POS_perc$Class[M_ALL_0.015_POS_perc$X.1==2]<-"Depdep"


M_ALL_IND_perc0.015<-M_ALL_IND_perc[which(M_ALL_IND_perc$KendalT.p.value>0.015),]
...
## DEPENDENT COUPLES
M1_DEP_0<-M_ALL_0.015_POS_perc[,c(15,57,69,4,68,18,25)]

colnames(M1_DEP_0)<-c("Meteo","Hydrology","Climatology","KendallTau","Class","X.1","Distance")


## DEPENDENT COUPLES
M1_IND_0<-M_ALL_IND_perc0.015[,c(15,57,69,4,68,18,25)]
...
M1_DEP_0[,3]<- M_ALL_0.015_POS_perc$SAAR_61.90
M1_IND_0[,3]<- M_ALL_IND_perc0.015$SAAR_61.90
...
setwd("C:/Users/39349/Documents/Regional/Plot/DDMAP/1606")
pdf(file=paste("GROUP_ALL_3dPLOT_0015_ALTITUDE.pdf"),width=9, height=9)  

xxx<-1


for(hh in 2:9){
  k<-combinations(9, hh, v=c(50,52,53,54,55,56,57,58,60), set=TRUE, repeats.allowed=FALSE)
  
  for(ff in 1:nrow(k)){
    
    Density_perc$CODE[xxx]<-paste(hh,ff)
    Density_perc$Selected[xxx]<-paste(colnames(M_ALL_0.015_POS_perc)[c(k[ff,])], collapse = ' , ')
    colnames(M_ALL_0.015_POS_perc)[c(k[ff,])]
    
    M1_DEP_0[,2]<- apply(M_ALL_0.015_POS_perc[,c(k[ff,])],1,mean)   
    M1_IND_0[,2]<- apply(M_ALL_IND_perc0.015[,c(k[ff,])],1,mean)   
    ...
    title=print(paste0("Hydrological Indexes selected: ",colnames(M_ALL_0.015_POS_perc[,c(k[ff,])])))
    ...
    ##############depandind
    nam0<-paste(colnames(M_ALL_IND_perc)[c(k[ff,])], collapse = ' , ')
...
write.table(Density_perc, file="PercentageNAMES_0015_v2.txt")


par(mfrow=c(2,2))

boxplot(M1_DEP$Climatology[M1_DEP$Class=="Depdep"],M1_DEP$Climatology[M1_DEP$Class=="Depind"]
        ,names=c("DepDep","DepInd"), main="Clima")

boxplot(M1_DEP$Meteo[M1_DEP$Class=="Depdep"],M1_DEP$Meteo[M1_DEP$Class=="Depind"]
        ,names=c("DepDep","DepInd"), main="Meteo")

boxplot(M1_DEP$Hydrology[M1_DEP$Class=="Depdep"],M1_DEP$Hydrology[M1_DEP$Class=="Depind"]
        ,names=c("DepDep","DepInd"), main="Hydro")

boxplot(M1_DEP$KendallTau[M1_DEP$Class=="Depdep"],M1_DEP$KendallTau[M1_DEP$Class=="Depind"]
        ,names=c("DepDep","DepInd"), main="KendallTau")
...
boxplot(M1_DEP$Climatology[M1_DEP$Class=="Depdep"],M1_DEP$Climatology[M1_DEP$Class=="Depind"],M1_DEP$Climatology[M1_DEP$Class=="High Syn"]
        ,names=c("DepDep","DepInd","High Syn"), main="Clima")
