...
{
  if(M_ALL_0.015_POS_perc$KendalT.p.value[i]<=pvalue){M_ALL_0.015_POS_perc$X[i]<-2}
...
M_ALL_IND_perc0.015<-M_ALL_IND_perc[which(M_ALL_IND_perc$KendalT.p.value>pvalue),]
...
for(hh in 1:8){
  ...
  for(ff in 1:nrow(k)){
    ...
    Density_perc$Selected[xxx]<-paste(colnames(M_ALL_0.015_POS_perc)[c(50, k[ff,])], collapse = ' , ')
    colnames(M_ALL_0.015_POS_perc)[c(50, k[ff,])]
    
    M1_DEP_0[,2]<- apply(M_ALL_0.015_POS_perc[,c(50, k[ff,])],1,mean)   
    M1_IND_0[,2]<- apply(M_ALL_IND_perc0.015[,c(50, k[ff,])],1,mean)   
    ...
    title=print(paste0("Hydrological Indexes selected: ",colnames(M_ALL_0.015_POS_perc[,c(50, k[ff,])])))
    ...
    ##############depandind
    nam0<-paste(colnames(M_ALL_0.015_POS_perc[,c(50, k[ff,])]), collapse = ' , ')
