...
pdf(file=paste0("GROUP_ALL_FIXED3Area",pvalue,"_Lagtime_",Lag_time,".pdf"),width=9, height=9)  

xxx<-1

Predictors<-c("Arable.horticultural", "Grassland", "Mountain.heath.bog","Urban.extent","ASPBAR..Â..",         
              "DPSBAR..m.km.","PROPWET","LDP..km.","Catchment.area")

v1<- match(Predictors,colnames(M_ALL_0.01_perc))

 
for(hh in 4:9){
  k<-combinations(9, hh, v=c(33,40,43,v1), set=TRUE, repeats.allowed=FALSE)
    ff<-1
  for(ff in 1:nrow(k)){
    ...
    M1_DEP_0[,2]<- apply(M_ALL_0.01_POS_perc[,c(k[ff,])],1,mean)   
    M1_IND_0[,2]<- apply(M_ALL_IND_perc0.01[,c(k[ff,])],1,mean)   
    ...
    title=print(paste0("Hydrological Indexes selected: ",colnames(M_ALL_0.01_POS_perc[,c(k[ff,])])))
