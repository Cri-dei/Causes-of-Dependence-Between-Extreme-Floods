...
POT_matrix<-as.data.frame(matrix(, nrow = nrow(Couples_investigate), ncol = 20))
...
colnames(POT_matrix)<-c("CODE", "ID_Station_1",  "ID_Station_2",
                        ...
                        "MAX_ANNUAL_KT", "MAX_ANNUAL_pvalue", "OLD_KT", "OLD_pvalue")
...
for (yy in 1:nrow(Couples_investigate))
{
    ...
    Available_stday<-read.table("Elencofile.txt")
    
  if(any(ID_Station_1==Available_stday) && any(ID_Station_2==Available_stday))
  {
    DAILY_Station_1<- read.csv(paste0("nrfa-public-17_",ID_Station_1,"_gdf.csv"),skip=20,sep=",", header=FALSE)
    colnames(DAILY_Station_1)<-c("Data_1","Discharge_1")
    
    #Read daily discharge for Station 2
    
    DAILY_Station_2<- read.csv(paste0("nrfa-public-17_",ID_Station_2,"_gdf.csv"),skip=20,sep=",", header=FALSE)
    ...
    if(length(unique( Final_Multi_merged$Var1))>5){
      ...
      POT_matrix$OLD_KT[xx]<- DEPdep$KendalT.value[yy]
      POT_matrix$OLD_pvalue[xx]<- DEPdep$KendalT.p.value[yy]    
      ...
      List_couple[[xx]]<-list(Dep_12,Dep_21)
...
if(TYPE_ANALYSIS== "POT mixed approach")
  
{ setwd(paste0(path,"/Results/POT_mixedapproach"))
  
  save.image(paste0("POT_Mix_lag",Lag_time,".RData"))
  save(POT_matrix,file=paste0("Final_matrix_POTMix_lag",Lag_time,".RData"))
  
  write.table(POT_matrix,paste0("POT_Mix_lag",Lag_time,".csv"), row.names=F, col.names=T)
