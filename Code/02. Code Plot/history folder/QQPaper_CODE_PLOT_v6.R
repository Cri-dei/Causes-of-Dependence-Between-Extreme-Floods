...
GG_5<-ggplot(data=M_ALL ,aes(x=M_ALL[,25],y= M_ALL[,15],color = as.factor(X)))+geom_point(alpha = 0.3, size=0.9) + theme_bw()+ 
...
GG_6<-ggplot(data=M_ALL ,aes(x=M_ALL[,25],y= M_ALL[,4],color = as.factor(X)))+geom_point(alpha = 0.3, size=0.9) + theme_bw()+ 
...
GG_update<-ggarrange(GG_11,GG_22,GG_3,GG_44,GG_5,GG_6,ncol=3,nrow=2, labels=c("a)","b)","c)","d)","e)","f)") ,font.label = list(size = 13)) 
...
Histogram<-"C:/Users/39349/Documents/Regional/Plot/HIST-1.jpeg"
KT_distance<-"C:/Users/39349/Documents/Regional/Plot/KT-DIS-1.jpeg"
...
ggarrange(KT_dist,Hist,ncol=1,nrow=2,labels=c("a)","b)"))
