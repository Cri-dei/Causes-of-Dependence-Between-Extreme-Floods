###############AUTHORS: Cristina Deidda and Leila Rahimi #######################
##################Last modify: 03/06/08 ########################################
#Creation of Couples and computation of Kendall Tau
...
Syncrony<- which( Real_matrix[,4] <= Real_matrix[,8]+10 & Real_matrix[,4] >= Real_matrix[,8]-10 )
...
save(KT_matrix, Difference, file="InitialKT.RData")

setwd("D:/PROJECTS/Regional/RESULTS/UK/100619")

write.table(KT_matrix, file="UK__CORR_LAG10_1006.csv", sep=";",dec=",", na="", row.names=F, col.names=T)

write.table(Difference, file="UK_LAG10_1006.csv", sep=";",dec=",", na="", row.names=F, col.names=T)
...
write.table(ELAB_KT_matrix, file="ELAB_KT_matrixLAG.csv", sep=";",dec=",", na="", row.names=F, col.names=T)
